    ...
    <div className="flex-1 overflow-y-auto px-6 py-4 space-y-6 bg-background">
      {messages.map(msg => (
        msg.sender === 'user' && editingId !== msg.id ? (
          <div key={msg.id} className="flex flex-row items-end justify-end">
            <button
              className="p-1 rounded hover:bg-accent mr-2 self-center"
              title="Edit"
              onClick={() => startEdit(msg.id, msg.text)}
            >
              <Pencil className="w-4 h-4" />
            </button>
            <div className="relative rounded-lg px-4 py-2 max-w-[70%] bg-primary text-primary-foreground">
              {msg.text}
            </div>
          </div>
        ) : (
